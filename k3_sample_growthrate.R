...
starting_pop_sizes <- c(500,1000,5000,10000,100000,250000,500000) #get appropriate growth rates for range of starting pop sizes
Nfinal <- 500000
growth_rates <- sapply(starting_pop_sizes,function(e) (log(e/Nfinal))/40000) #Nt=Nfinal*exp(r*t) ... rearrange to r=(ln(Nt/Nfinal))/t

par <- par <- readLines("fsc_params/sim_k3.par")

for(i in growth_rates){
  par[grep("Growth rates",par)+2] <- i
  par[grep("Growth rates",par)+3] <- i
  writeLines(par,paste0("./fsc_params/growth_",i,".par"))
  Sys.sleep(1) 
  system(paste0("cd ~/Dropbox/structure_simulations/;
       /Applications/fsc_mac64/fsc25221 -i ",paste0("./fsc_params/growth_",i,".par")," -n 10;
       mv ",paste0("growth_",i,"/*")," sample_growth/sim/ ;
       rm -r ",paste0("growth_",i))) #edit sim.par (or new par) as needed.
}
...
mainparams_path <- "/media/burke/bigMac/Dropbox/structure_simulations/sample_growth/str_params/mainparams.txt"
extraparams_path <- "/media/burke/bigMac/Dropbox/structure_simulations/sample_growth/str_params/extraparams.txt"
params_dir <- "/media/burke/bigMac/Dropbox/structure_simulations/sample_growth/str_params/"
str_in <- "/media/burke/bigMac/Dropbox/structure_simulations/sample_growth/str_in/"
str_out <- "/media/burke/bigMac/Dropbox/structure_simulations/sample_growth/str_out/"
...
for(i in 1:length(files)){
  ...
  growth <- strsplit(files[i],"_") %>% unlist() %>% .[3] %>% as.numeric()
...
names(sum_stats) <- c("growth_rates","run","maf","q_dist")

strplotdata <- do.call(rbind,q_matrices)
meltq <- melt(strplotdata,id.vars=c("id","pop","growth_rate","run","maf"))
ggplot(data=subset(meltq,maf==2),aes(x=id,y=value,fill=variable))+facet_grid(run~growth_rate)+
...
ggplot(data=sum_stats,aes(y=factor(growth_rates),x=q_dist,fill=factor(maf)))+
  theme_minimal()+
  xlim(0,1)+
